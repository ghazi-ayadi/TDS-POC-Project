...
def video_stream(niryo_one_client):
    ...
    while True:
        ...
        for idx, (u, v) in enumerate(centers):
            ...
            # Store coordinates in respective variables
            if idx == 0:
                x1, y1, z1 = X, Y, Z_world
            elif idx == 1:
                x2, y2, z2 = X, Y, Z_world
            elif idx == 2:
                x3, y3, z3 = X, Y, Z_world
            elif idx == 3:
                x4, y4, z4 = X, Y, Z_world

            print(f"Circle {idx+1} detected at (u, v): ({u}, {v}) --> World (X, Y, Z): ({X:.2f}, {Y:.2f}, {Z_world:.2f})")
    ...
    print(f"Circle 1 coordinates: ({x1}, {y1}, {z1})")
    print(f"Circle 2 coordinates: ({x2}, {y2}, {z2})")
    print(f"Circle 3 coordinates: ({x3}, {y3}, {z3})")
    print(f"Circle 4 coordinates: ({x4}, {y4}, {z4})")
